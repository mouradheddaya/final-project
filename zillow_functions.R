...
buyer_seller <- read.csv("final-project/BuyerSellerIndex_City.csv", stringsAsFactors = FALSE)
neighborhood_all <- read.csv("final-project/Neighborhood_MedianListingPrice_AllHomes.csv", stringsAsFactors = FALSE)
neighborhood_bottom <- read.csv("final-project/Neighborhood_MedianListingPrice_BottomTier.csv", stringsAsFactors = FALSE)
neighborhood_top <- read.csv("final-project/Neighborhood_MedianListingPrice_TopTier.csv", stringsAsFactors = FALSE)
sales_foreclosed <- read.csv("final-project/SalesPrevForeclosed_Share_Neighborhood.csv", stringsAsFactors = FALSE)

test_code <-buyer_seller %>% filter(State == "WA" & RegionName == "Tacoma")
cbind(State, test_code$RegionName, geocode(as.character(test_code$RegionName)))

# lat_lon <- cbind(buyer_seller, geocode(as.character(buyer_seller$RegionName)))
# write.csv(lat_lon, file = "final-project/lat_lon.csv")

# Plots the buyer seller index for a given state
buyer_index  <- function(data, state){
  selected_state <- map_data("state", region= "washington")
  ggplot(data = selected_state, aes(x=long, y=lat, group = group)) + 
        coord_quickmap() + geom_point(data = lat_lon, aes(x = lon, y = lat, size = BuyerSellerIndex), color = "orange")
}
